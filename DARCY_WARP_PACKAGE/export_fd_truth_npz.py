...
SERIAL_CELL_THRESHOLD = 1_000_000
...
def _run_case_variant(
    ...
    heads, elapsed = run_fd_truth_forward(
        nx=nx,
        ny=ny,
        dx=dx_truth,
        T_truth=t_field,
    ...
    np.savez_compressed(
        ...
        seed=np.int32(seed),
        elapsed_seconds=np.asarray(elapsed, dtype=float_dtype),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Export FD truth heads to compressed NPZ.")
    parser.add_argument("--out_dir", type=str, default=str(data_store.joinpath("fd_truth_npz")))
    ...
    for label in labels:
        ...
        for isotropic in isotropic_flags:
            for ghb in ghb_flags:
                out_name = f"fd_truth_{label}_ghb_{ghb}_t_isotropic_{isotropic}.npz"
    ...
    serial_jobs: list[tuple] = []
    parallel_jobs: list[tuple] = []
    for job in jobs:
        nx = int(job[1])
        ny = int(job[2])
        if nx * ny > SERIAL_CELL_THRESHOLD:
    ...
    if serial_jobs:
        print(
            "Forcing serial FD truth for "
            f"{len(serial_jobs)} case(s) > {SERIAL_CELL_THRESHOLD} cells."
        )
    ...
    if len(parallel_jobs) == 1:
        out_path = _run_case_variant(*parallel_jobs[0])
        print(f"Wrote {out_path}")
        return 0

    ctx = mp.get_context("spawn")
    with ProcessPoolExecutor(max_workers=int(args.workers), mp_context=ctx) as pool:
        futures = [pool.submit(_run_case_variant, *job) for job in parallel_jobs]

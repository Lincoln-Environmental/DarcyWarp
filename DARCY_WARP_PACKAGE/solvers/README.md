...
| `unconfined_semismooth_newton_kcycle` | unconfined Newton (FGMRES + K-cycle preconditioner) | steady or multi-period transient | production alternative |
...
experimental backend is selected, and the multi-period transient dispatcher
(`transient_unconfined.py`) gates on `supports_production_period_driver`.
Newton and explicitly selected FAS runs use the alternate nonlinear period
driver in `transient_experimental.py`; the filename and diagnostic field names
are retained for compatibility, while only FAS remains experimental. Their shared nonlinear

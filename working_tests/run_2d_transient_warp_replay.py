...
* Large transient unconfined replays should start from a confined steady-state
  warm start rather than the raw geometric initial head. The preferred artifact
  field is ``confined_steady_head`` generated by the MF6 truth workflow with the
  same spatial fields and representative recharge. This keeps MF6 and Warp
  ...
  re-solving the warm start with Warp (``confined_steady_warp``) while MF6 used
  ``confined_steady_mf6`` (or ``artifact_initial``) conflates warm-start error
  with transient error and is rejected by :func:`validate_warm_start_comparability`
  unless ``allow_warm_start_mismatch=True``.
...
def select_artifact_warm_start(
    ...
    if mode not in {WARM_START_ARTIFACT_INITIAL, WARM_START_CONFINED_STEADY_MF6}:
        raise ValueError(
            "artifact warm_start_mode must be 'artifact_initial' or 'confined_steady_mf6'."
        )
...
def artifact_warm_start_provenance(artifact: dict) -> str | None:
    ...
    ``initial_head`` (``artifact_initial`` or ``confined_steady_mf6``). This is
    used to check that a Warp replay warm start is comparable to the MF6 truth.
...
def validate_warm_start_comparability(
    ...
    MF6 and Warp each solve their own confined steady warm start, so a Warp
    replay that warm-starts from a Warp confined solve (``confined_steady_warp``)
    is NOT comparable to MF6 heads that warm-started from an MF6 confined solve
    (``confined_steady_mf6``) or from the raw ``artifact_initial`` head. That
    mismatch conflates warm-start error with transient error and must be opted
    into explicitly.
    ...
    Only ``confined_steady_warp`` (the Warp re-solve) is rejected; starting from
    the artifact's own ``artifact_initial`` or ``confined_steady_mf6`` head is
    always comparable.
    """
    if artifact_warm_start is None:
        return
    if warp_warm_start_mode != WARM_START_CONFINED_STEADY_WARP:
        return
    if artifact_warm_start == WARM_START_CONFINED_STEADY_WARP:
        return  # not produced by the MF6 generator, but guard anyway
    if allow_warm_start_mismatch:
        return
    raise ValueError(
        f"Warp warm_start_mode='confined_steady_warp' re-solves the warm start with "
        f"Warp, but the MF6 artifact warm-start provenance is "
        f"'{artifact_warm_start}'. That makes the Warp-vs-MF6 head comparison "
        f"invalid (warm-start error is conflated with transient error). Use "
        f"warm_start_mode='{artifact_warm_start}' to start from the artifact's own "
        f"warm-start head, or pass allow_warm_start_mismatch=True to proceed anyway."
    )
...
def run_warp_transient_replay(
    ...
    if warm_start_head is not None:
    ...
    elif warm_start_mode == WARM_START_CONFINED_STEADY_MF6:
        raise ValueError(
            "warm_start_mode='confined_steady_mf6' requires warm_start_head. "
            "Use run_replay_from_artifact so it can read artifact['confined_steady_head']."
        )
    else:
        replay_start_head = validate_warm_start_head(
            head=initial_head,
        ...
        warm_start_used = WARM_START_CONFINED_STEADY_WARP

    if (
        formulation == FORMULATION_UNCONFINED
        and warm_start_used != WARM_START_ARTIFACT_INITIAL
        and "unconfined_startup_mode" not in explicit_control_keys
    ):
        # The replay already starts from a confined steady head, so the solver's
        # internal confined pre-solve would repeat that startup solve inside the
        # nonlinear transient step and can destabilize large-grid replays.
        controls["unconfined_startup_mode"] = "initial_head"
    ...
    if formulation == FORMULATION_CONFINED:
        ...
        initial_transmissivity = _initial_transmissivity(k, replay_start_head, bottom, active, min_sat)
    ...
    with WarpDarcySolver(
        ...
        if warm_start_mode == WARM_START_CONFINED_STEADY_WARP and warm_start_head is None:
            ...
            head_warm, _info_warm = solver.solve(
                ...
                return_info=True,
                max_cycles=int(controls.get("max_cycles", 40)),
                max_levels=int(controls.get("max_levels", 5)),
                min_coarse_cells=int(controls.get("min_coarse_cells", 1)),
                check_every_no=int(controls.get("check_every_no", 1)),
                rel_tol=float(controls.get("rel_tol", 5.0e-7)),
                abs_tol_min=float(controls.get("abs_tol_min", 5.0e-7)),
                dh_rms_tol=controls.get("dh_rms_tol"),
            )
...
def run_replay_from_artifact(
    ...
    By default the replay starts from the artifact's own ``confined_steady_head``
    (``confined_steady_mf6``) and builds an MF6-matching convertible storativity
    field (``mf6_convertible``: ``S = Sy + Ss*saturated_thickness``). Starting
    from a Warp re-solved warm start (``confined_steady_warp``) while the artifact
    was produced with a different warm start is rejected unless
    ...
    if warm_start_mode == WARM_START_CONFINED_STEADY_WARP:
...
if __name__ == "__main__":
    ...
    # Default to the artifact's own confined steady head so the Warp-vs-MF6
    # comparison is valid (re-solving the warm start with Warp would mismatch the
    # MF6 provenance). Override to confined_steady_warp only with
    # allow_warm_start_mismatch=True.

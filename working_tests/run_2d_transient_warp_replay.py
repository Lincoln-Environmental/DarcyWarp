...
def run_warp_transient_replay(
    ...
    recharge_rates:
        ...
        time unit as ``dt``.
    sy:
        Specific yield used as the phreatic storage coefficient for unconfined
        replay.
    ss:
        Specific storage used for confined replay. The replay converts this to
        storativity as ``Ss * saturated_thickness`` using the warm-start head.
    dt:
    ...
    if formulation == FORMULATION_CONFINED:
        ...
        storage_coeff = _confined_storage_coeff(
            ss=float(ss),
            top=top,
            bottom=bottom,
            active=active,
    ...
    return {
        ...
        "device": device,
        "storage_coeff": storage_coeff,
        "storage_coeff_kind": "sy" if formulation == FORMULATION_UNCONFINED else "ss_times_cell_thickness",
        "dt": float(dt),
        "formulation": formulation,
...
def run_replay_from_artifact(
    ...
    if formulation == FORMULATION_UNCONFINED:
        print(f"  formulation: unconfined; Sy={sy} (phreatic storage), Ss={ss} not modelled")
    else:
        print(f"  formulation: confined; Ss={ss} converted to storativity via warm-start saturated thickness")
    ...
    np.savez_compressed(
        ...
        storage_coeff_kind=np.asarray(warp_result["storage_coeff_kind"]),
    ...
    summary = {
        ...
        "storage": {
            "sy": sy,
            "ss": ss,
            "warp_storage_coeff": warp_result["storage_coeff"],
            "warp_storage_coeff_kind": warp_result["storage_coeff_kind"],
        },
        "warm_start": {
...
if __name__ == "__main__":
    ...
    warm_start_mode = WARM_START_CONFINED_STEADY_WARP
    formulation = FORMULATION_UNCONFINED
